...
function getFilteredTotalGoals(minGoals, teamPlayers) {
    return teamPlayers.filter(player => player.goals >= minGoals).reduce((total, player) => total + player.goals, 0);
}

const result1 = getFilteredTotalGoals(5, players);
console.log(`Tổng các bàn thắng của các cầu thủ đạt đủ chỉ tiêu là : ${result1}`);

const result2 = getFilteredTotalGoals(50, players);
